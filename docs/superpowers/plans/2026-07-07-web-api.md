...
- Produces:
  - `apt.api.deps`: `current_user(request) -> User | None` (cookie → verify_session → UserRepo.get); `require_user(request) -> User` (401 when None); `require_admin(request) -> User` (403 unless email in config.admin_emails). (Routers read `request.app.state` directly — no separate state helper.)
